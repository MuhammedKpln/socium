...
export function Followings(props: IProps) {
  ...
  const renderItem = useCallback(
    ({ item }: { item: IFollowers }) => {
      console.log(myFollowings?.data?.getUserFollowings)
      const myFollowingsIds = myFollowings?.data?.getUserFollowings.filter(
        v => v.actor.id === item.actor.id,
      )

      console.log(myFollowingsIds)
      const isFollowing = myFollowingsIds && myFollowingsIds?.length > 0

      return (
        <ListItem onPress={() => onPressUser(item.actor.username)}>
          <ListItem.Part left>
            <Avatar userAvatar={item.actor.avatar} />
          </ListItem.Part>
          ...
          {localUser?.id !== item.actor.id && (
            <ListItem.Part right>
              <Button
                ...
                onPress={
                  !isFollowing
                    ? () => onPressFollowUser(item.actor.id)
                    : () => onPressUnfollowUser(item.actor.id)
                }
              />
            </ListItem.Part>
          )}
  ...
  return (
    <Page>
      <SkeletonView
        ...
        times={data?.getUserFollowings.length || 5}
        listProps={{
